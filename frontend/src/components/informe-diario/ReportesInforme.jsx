...
import {
  ...
  Edit3, ChevronRight, FileSpreadsheet,
} from 'lucide-react';
import { exportarInformeExcel } from '@/utils/exportInformeExcel';
...
const ACTIVIDADES_DEFAULT = [
  ...
  { id: 'sst',       categoria: 'GESTIÓN SST',                                                    color: '#10b981', actividades: ['Seguimiento al ingreso de personal a la SE La Loma', 'Charla "Uso adecuado de las herramientas de trabajo"', 'Delimitación y señalización de las áreas', 'Orden y aseo en las áreas', 'Seguimiento medidas de prevención y control SST', 'Elaboración de informe diario e informe semanal'] },
  { id: 'ambiental', categoria: 'ACTIVIDADES AMBIENTALES Y SOCIALES',                             color: '#22c55e', actividades: ['Jornadas de orden y aseo de las áreas de trabajo', 'Delimitación y señalización de las áreas'] },
];
    ...
    <div style={{ color: '#1e293b', fontSize: 13 }}>
      ...
      <div style={s.strip}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          ...
          >
            <FileSpreadsheet size={16} style={{ marginRight: 7 }} />
      ...
      <Section title={<><Wrench size={13} style={{ marginRight: 6 }} />MAQUINARIA — EQUIPOS — HERRAMIENTAS Y VEHÍCULOS</>}>
        <div style={s.tableWrap}>
          <table style={s.table}>
            ...
            <tbody>
              {maquinaria.map(m => (
                <tr key={m.id} style={s.tr}>
                  ...
                  <Td center>
                    {editMaq === m.id
                      ? <input type="number" min={0} style={s.inlineNum} value={m.cantidad}
                          onChange={e => updateMaq(m.id, 'cantidad', parseInt(e.target.value) || 0)} autoFocus />
                      : <span style={m.cantidad > 0 ? s.numOn : s.numOff}>{m.cantidad}</span>
                    }
                  </Td>
                  <Td>
                    {editMaq === m.id
                      ? <input style={s.inlineText} value={m.empresa}
                          onChange={e => updateMaq(m.id, 'empresa', e.target.value)} placeholder="Empresa..." />
                      : <span style={s.empresa}>{m.empresa || '—'}</span>
                    }
                  </Td>
                  <Td>
                    {editMaq === m.id
                      ? <input style={s.inlineText} value={m.notas}
                          onChange={e => updateMaq(m.id, 'notas', e.target.value)} placeholder="Notas..." />
            ...
            <tfoot>
              <tr>
                <td colSpan={4} style={s.tfoot}>TOTAL EQUIPOS</td>
                <td style={{ ...s.tfoot, textAlign: 'center', color: '#0d9488', fontWeight: 800 }}>{totalMaquinaria}</td>
              </tr>
            </tfoot>
          </table>
        </div>
          ...
          <table style={s.table}>
            ...
            <tfoot>
              <tr>
                ...
                <td style={{ ...s.tfoot, textAlign: 'center', color: '#0d9488', fontWeight: 800 }}>{totalPersonal}</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      {/* ── ACTIVIDADES DEL DÍA — SOLO LECTURA ──────────────────── */}
...
const s = {
  ...
  tog:        { padding: '3px 10px', fontSize: 11, fontWeight: 700, border: '1px solid #e2e8f0', borderRadius: 5, background: 'transparent', color: '#94a3b8', cursor: 'pointer' },
