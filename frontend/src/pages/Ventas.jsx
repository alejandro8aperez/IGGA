    ...
    <div className="p-8 font-sans" style={s.page}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
        <div>
          ...
          <p className="text-slate-400" style={{ marginTop: '0.5rem' }}>Control de pedidos y ventas.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
        <StatCard icon={<DollarSign />} title="Ingresos" value={'$' + Number(totalVentas).toLocaleString('es-CO')} color="#10B981" />
        <StatCard icon={<Users />} title="Clientes" value={totalClientes} color="#3B82F6" />
        <StatCard icon={<TrendingUp />} title="Pedidos" value={ordenes.length} color="#6366F1" />
        <StatCard icon={<Clock />} title="Estado" value="Activo" color="#F59E0B" />
      </div>
      ...
      <div style={s.card}>
        <div className="overflow-x-auto">
          <table style={s.table}>
            ...
            <tbody>
              {ordenes
                .filter(o => (o.cliente_nombre || '').toLowerCase().includes(searchTerm.toLowerCase()))
                .map((orden) => (
                  <tr key={orden.id}>
                    ...
                    <td style={s.td}>${Number(orden.total || 0).toLocaleString('es-CO')}</td>
                    <td style={s.td}>
                      <div className="flex gap-2">
                        <button title="Editar" onClick={() => openOrdModal(orden)} style={{ background: 'transparent', border: 'none', color: '#818CF8', cursor: 'pointer' }}>
                          <Eye size={18} />
                        </button>
                        <button title="Eliminar" onClick={() => eliminarOrden(orden.id)} style={{ background: 'transparent', border: 'none', color: '#EF4444', cursor: 'pointer' }}>
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
      ...
      {isOrdModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}>
          <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '900px', maxHeight: '90vh', overflow: 'auto', padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700 }}>
                {currentOrd ? `Editar Pedido #${currentOrd.id}` : 'Nuevo Pedido'}
              </h2>
            ...
            <form onSubmit={handleOrdSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div>
                  ...
                  <select value={ordForm.cliente} onChange={(e) => setOrdForm({ ...ordForm, cliente: e.target.value })} style={s.input} required>
                ...
                <div>
                  ...
                  <input type="date" value={ordForm.fecha_entrega_esperada} onChange={(e) => setOrdForm({ ...ordForm, fecha_entrega_esperada: e.target.value })} style={s.input} />
                </div>
              </div>

              <div style={{ marginBottom: '1rem' }}>
                <button type="button" onClick={addProductoToOrden} style={{ background: '#10B981', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer' }}>
                  + Agregar Producto
                </button>
              </div>

              {ordForm.detalles.map((detalle, index) => (
                <div key={detalle.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: '1rem', marginBottom: '1rem' }}>
                  <select value={detalle.producto} onChange={(e) => updateProductoInOrden(index, 'producto', e.target.value)} style={s.input}>
                    <option value="">Producto</option>
                    {productos.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
                  </select>
                  <input type="number" value={detalle.cantidad} onChange={(e) => updateProductoInOrden(index, 'cantidad', e.target.value)} style={s.input} />
                  <input type="number" value={detalle.valor_unitario} onChange={(e) => updateProductoInOrden(index, 'valor_unitario', e.target.value)} style={s.input} />
                  <div style={{ display: 'flex', alignItems: 'center', fontWeight: 700 }}>
                    ${Number(detalle.valor_total).toLocaleString('es-CO')}
                  </div>
                  <button type="button" onClick={() => removeProductoFromOrden(index)} style={{ background: 'transparent', border: 'none', color: '#EF4444', cursor: 'pointer' }}>
                    <Trash2 size={18} />
                  </button>
                </div>
              ))}

              <div style={{ textAlign: 'right', fontSize: '1.25rem', fontWeight: 700, marginTop: '1rem' }}>
                Total: ${Number(ordForm.total).toLocaleString('es-CO')}
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                <button type="button" onClick={closeOrdModal} style={{ padding: '0.75rem 1.5rem', border: '1px solid #D1D5DB', borderRadius: '8px', background: 'white', cursor: 'pointer' }}>
                  Cancelar
                </button>
                <button type="submit" style={{ padding: '0.75rem 1.5rem', border: 'none', borderRadius: '8px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', cursor: 'pointer', fontWeight: 600 }}>
                  Guardar Pedido
                </button>
              </div>
            </form>
...
function StatCard({ icon, title, value, color }) {
  return (
    <div style={{ background: '#1E293B', padding: '1.5rem', borderRadius: '12px', border: '1px solid #334155', display: 'flex', alignItems: 'center', gap: '1rem' }}>
      <div style={{ background: color + '22', color, padding: '1rem', borderRadius: '10px' }}>
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background: statusCfg.c + '22', color: statusCfg.c, padding: '0.2rem 0.6rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700, border: `1px solid ${statusCfg.c}44` }}>
      {statusCfg.l}
    </span>
  );
}
